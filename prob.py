...
def extract_feature(text):
    ...
    ppls = np.nanmean([compute_perplexity(sent) for sent in sents], axis=0)
...
results_df = pd.DataFrame(results,  columns=['PAR', 'Task', 'org_ppl', 'rev_ppl', 'diff_ppl'])
results_df.to_csv('Features/ppl.csv', index=False)   
