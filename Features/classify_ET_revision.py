...
morphosyntax = pd.merge(pd.read_csv('wav_sim.csv')[['PAR', 'Task', 'TokenNum']],
                              ...
                              on=['PAR', 'Task'], how='inner')
morphosyntax = morphosyntax.drop(columns=['punct_DepR'])
...
def task_ablate(task_del):
    
    k2acc_dfs = pd.DataFrame()
    for i, task_merge in enumerate(task_merges):   
        k2acc = find_best_k(task_merge, all_features, clf, i+1, tasks=[t for t in tasks if t!=task_del])
...
feature_importance = []
for fold_i, task_merge in enumerate(task_merges): 
...
feature_if_mean['Group'] = feature_if_mean['Feature'].apply(map_feat_to_group)
        
# kurskal wallis
rm_results = pg.kruskal(data=feature_if_mean, dv='Importance', between='Group')
print()
print("Kurskal Wallis' test on feature importance")
print(rm_results)

# posthoc 
rm_posthoc = sp.posthoc_dscf(feature_if_mean, val_col='Importance', group_col='Group')
print(rm_posthoc)
...
bp = sns.boxplot(data=feature_if_mean, y='Importance', x='Group', ax=ax, 
                  palette='Pastel1')
...
# 8. ablation study
ablate_perf = fold_acc.copy()
ablate_perf.insert(0, 'Task_del', 'None')

for task in tasks:
    print()
    print('Ablate task:', task)
    ablate_perf_task = task_ablate(task)
    ablate_perf_task.insert(0, 'Task_del', task)
    ablate_perf = pd.concat([ablate_perf, ablate_perf_task]) 

ablate_perf.to_csv(out_dir + 'task_ablation.csv', index=False)

ablate_perf_avg = ablate_perf[ablate_perf['Fold']=='Avg']
ablate_perf_avg.index = range(len(ablate_perf_avg))

# differences in scores
baseline = ablate_perf_avg[ablate_perf_avg['Task_del'] == 'None']
diff_df = ablate_perf_avg.copy()
for col in ablate_perf_avg.columns[3:]:  
    diff_df[col] = ablate_perf_avg[col] - baseline[col].values[0]

diff_df = diff_df.drop(index=0)[['Task_del', 'test_precision', 'test_recall', 
                                  'test_f1', 'test_acc']]

diff_df = diff_df.melt(id_vars='Task_del', var_name='Metric', value_name='Differences')

fig, ax = plt.subplots(figsize=(10, 6))
sns.lineplot(data=diff_df, x='Task_del', y='Differences', hue='Metric', marker='o')
ax.set_title('PRFA performance changes by ablating tasks on test set', fontsize=14)
ax.set_xlabel('Task_del', fontsize=12)
ax.set_ylabel('Difference', fontsize=12)
ax.legend(title='Metric', fontsize=10)

fig.savefig(out_dir + 'task_ablation.tif', dpi=300, bbox_inches='tight') 
fig.savefig(out_dir + 'task_ablation.png', dpi=300, bbox_inches='tight')

# 9. features with importance top 50 correlation analysis 
# feats_impt = feature_if_mean[feature_if_mean['Importance'] > 0.005].Feature
feats_impt = feature_if_mean.sort_values(by="Importance", ascending=False).head(50).Feature
...
# select
pivot_df_sel = pivot_df.loc[:, (pivot_df != 0).any(axis=0)]
rps_sel = rps[pivot_df_sel.columns]

fig, ax = plt.subplots(figsize=(16, 16))
    
sns.heatmap(pivot_df_sel.T, fmt="", annot=rps_sel.T, annot_kws={"size": 13}, 
            cbar=True, 
            cbar_kws={"shrink": 0.75, "orientation": 'vertical'}, 
            vmin = -1, vmax = 1, 
            square=True, linewidth=.5, 
...
fig.savefig(out_dir + 'panss_corr_sel.tif', dpi=300, bbox_inches='tight') 
fig.savefig(out_dir + 'panss_corr_sel.png', dpi=300, bbox_inches='tight') 
panss_corrs.to_csv(out_dir + 'panss_corr.csv', index=False) 

# 10. factor analysis 
feature_matrix = StandardScaler().fit_transform(task_merge[feats_impt].fillna(0).values)

# 10.1 PC-based parallel analysis 
# https://stackoverflow.com/a/70918704
pca = PCA(len(feats_impt)-1)
pca.fit(feature_matrix)
transformedShapeMatrix = pca.transform(feature_matrix)

# random eigenvalues
np.random.seed(1234)
random_eigenvalues = np.zeros(len(feats_impt)-1)
for i in range(100):
    random_shapeMatrix = pd.DataFrame(np.random.normal(0, 1, list(feature_matrix.shape)))
    pca_random = PCA(len(feats_impt)-1)
...
for dv_feat in tqdm(feats_impt, desc='mixed_lm'):
    ...
    model = mixedlm(formula, df, groups=df['PAR'], re_formula="~Task", missing="drop")
    result = model.fit()
    ...
    mixed_lm_results.append([dv_feat, z, p, ci_l, ci_u])

mixed_lm_results = pd.DataFrame(mixed_lm_results, columns=['Feat', 'Z', 'p', 'CI_lower', 'CI_upper'])
mixed_lm_results.insert(3, 'q', multitest.fdrcorrection(mixed_lm_results['p'])[1])
mixed_lm_results['Set'] = mixed_lm_results['Feat'].apply(lambda x: next((key for key, values in dict_feat.items() if x in values), None))
mixed_lm_results['Z_abs'] = mixed_lm_results['Z'].apply(abs)
mixed_lm_results = mixed_lm_results.sort_values(by='Z_abs', ascending=True).reset_index(drop=True) 
mixed_lm_results.to_csv(out_dir + 'mixed_lm.csv', index=False)
...
# Plot error bar
ax.errorbar(mixed_lm_results['Z'], range(len(mixed_lm_results)), 
              xerr=[mixed_lm_results['Z'] - mixed_lm_results['CI_lower'], 
                    mixed_lm_results['CI_upper'] - mixed_lm_results['Z']], 
              fmt='o', capsize=5, ms=3, elinewidth=2, markeredgewidth=2,
              ecolor='#e18727ff', mfc='#7e6148ff', mec='#7e6148ff')

ax.set_yticks(range(len(mixed_lm_results)))
ax.set_yticklabels(mixed_lm_results['Feat'])
ax.set_xlabel('Z values')
ax.set_title('Z values with confidence interval', fontsize=14)
...
# significant ones
mixed_lm_results_sel = mixed_lm_results[mixed_lm_results['q'] < 0.05]
...
# Plot error bar
ax.errorbar(mixed_lm_results_sel['Z'], range(len(mixed_lm_results_sel)), 
              xerr=[mixed_lm_results_sel['Z'] - mixed_lm_results_sel['CI_lower'], 
                    mixed_lm_results_sel['CI_upper'] - mixed_lm_results_sel['Z']], 
              fmt='o', capsize=5, ms=3, elinewidth=2, markeredgewidth=2,
              ecolor='#e18727ff', mfc='#7e6148ff', mec='#7e6148ff')

ax.set_yticks(range(len(mixed_lm_results_sel)))
ax.set_yticklabels(mixed_lm_results_sel['Feat'])
ax.set_xlabel('Z values')
ax.set_title('Z values with confidence interval', fontsize=14)
ax.axvline(0, color='gray', linestyle='dashed', linewidth=2)

fig.savefig(out_dir + 'group_compare_Z_sig.tif', dpi=300, bbox_inches='tight') 
fig.savefig(out_dir + 'group_compare_Z_sig.png', dpi=300, bbox_inches='tight') 
